...
- Phase 4 ist erweitert: Analyseantworten und Blazor-UI zeigen eine Zusammenfassung der Chunk-Qualitaet.
...
- Phase 5 ist erweitert: Generierte Roadmap-Wochen koennen in API und Blazor-UI strukturiert bearbeitet und gespeichert werden.
...
- Phase 6 ist erweitert: Wissenselemente speichern eine Review-Historie fuer Review-, Freigabe- und Ablehnungsaktionen; Blazor, JSON und SQLite-Persistenz zeigen bzw. erhalten diese Historie.
- Phase 6 ist erweitert: Analysierte Dokumentversionen koennen per API und Blazor-UI mit geaenderten, neuen, entfernten und unveraenderten Chunks verglichen werden.
...
- Monitoring-Grundlage ist erweitert: API und Blazor-Operationsbereich zeigen eine zusammengefasste Betriebsuebersicht aus Health, Runtime-Metriken und Release-Readiness-Signalen.
- Betriebs-Grundlage ist erweitert: Backup, Restore, Backup-Vorschau und Release-Readiness schreiben strukturierte Betriebslogs.
...
- Release-Attestation-Grundlage ist erweitert: CI erzeugt ein `release-attestation-manifest.json` mit Hashes der Security-Nachweise und archiviert `security-inventory` mit 90 Tagen Retention.
