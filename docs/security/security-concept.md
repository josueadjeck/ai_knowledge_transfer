...
Runtime secrets such as AI provider keys, database credentials and future provider-specific credentials must be injected by an approved secret manager or hosting platform. `AKT_SECRET_STORE_MODE=SecretStore` makes provider, mount path and rotation-owner metadata visible in health and release readiness without exposing secret values. `MountedFiles` is the first automated retrieval provider. See `docs/security/secret-management.md`.
...
The CI pipeline runs restore, build, publish, tests, vulnerability checks, a deterministic static source security scan, a deterministic secret scan, build artifact file inventory generation, dependency inventory generation, SPDX dependency SBOM generation, CycloneDX build SBOM generation, release attestation manifest generation, Dockerfile container policy scanning, container builds, built image vulnerability scanning, container image metadata inventory, container image provenance generation and registry-backed signing policy scanning.
...
- Platform-specific registry push/sign/verify automation, long-term external evidence retention beyond CI artifacts, direct cloud-secret-manager SDK integrations and enterprise SAST remain follow-up work.
