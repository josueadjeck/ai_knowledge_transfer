...
The CI pipeline runs restore, build, publish, tests, vulnerability checks, a deterministic static source security scan, a deterministic secret scan, build artifact file inventory generation, dependency inventory generation, SPDX dependency SBOM generation, CycloneDX build SBOM generation, Dockerfile container policy scanning, container builds, built image vulnerability scanning, container image metadata inventory, container image provenance generation and registry-backed signing policy scanning.
...
- Platform-specific registry push/sign/verify automation, external SBOM attestation storage and retention, direct cloud-secret-manager SDK integrations and enterprise SAST remain follow-up work.
