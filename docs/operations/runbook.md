...
1. Run the CI workflow on `main` and confirm restore, build, publish, build artifact inventory, dependency inventory, SPDX SBOM generation, CycloneDX build SBOM generation, container policy scan, container image vulnerability scan, container image inventory, container image provenance, container signing policy scan, release attestation manifest generation, container build, tests, vulnerability check, static source security scan and secret scan passed.
...
The `Security inventory` and `Container image inventory` release checks are manual. Review the uploaded `security-inventory` artifact and confirm `build-artifact-inventory.json`, `dotnet-package-inventory.json`, `sbom.spdx.json`, `cyclonedx-build-sbom.json`, `release-attestation-manifest.json`, `vulnerable-packages.json`, `static-source-scan.json`, `container-policy-scan.json`, `container-vulnerability-scan-api.json`, `container-vulnerability-scan-web.json`, `container-images.json`, `container-provenance.json` and `container-signing-policy.json` are present for the release candidate.
...
The `Secret management review` check is manual for environment-based local runs. In `SecretStore` mode it passes only when provider and rotation-owner metadata are configured. For `MountedFiles`, `AKT_SECRET_MOUNT_PATH` is also required and the application reads runtime secrets from files with matching setting names.
